...
public class TeacherService {
    ...
    public TeacherDTO setLoginidAndPassword(TeacherDTO teacherDTO) {
        ...
        String first4 = directorRepository.findById(teacherDTO.getDirectorId()).get().getLoginid().substring(0,4);
        // 학원 선생님 수 구하기 (남는 앞자리 0으로 채우기)
        String last4 = String.format("%04d", teacherRepository.findAllByDirectorId(teacherDTO.getDirectorId()).size() + 1); //! NullException
    ...
    public void regist(TeacherDTO teacherDTO) {
        ...
        TeacherEntity teacherEntity = TeacherEntity.toTeacherEntity(teacherDTO, directorRepository.findById(teacherDTO.getDirectorId()).get());
        teacherRepository.save(teacherEntity);
    }

    public MailDTO getMailDTOByRegist(TeacherDTO teacherDTO) {
        DirectorEntity directorEntity = directorRepository.findById(teacherDTO.getDirectorId()).get();
