...
public interface MessageRepository extends JpaRepository<MessageEntity, Long> {
    // 선생 받은쪽지 페이징 조회
    @Query("SELECT m FROM MessageEntity m WHERE m.teacherEntity.id = :teacherId and m.sender = 'S'")
    Page<MessageEntity> findAllReceiveByTeacherId(@Param("teacherId") Long teacherId, Pageable pageable);

    // 선생 보낸쪽지 페이징 조회
    @Query("SELECT m FROM MessageEntity m WHERE m.teacherEntity.id = :teacherId and m.sender = 'T'")
    Page<MessageEntity> findAllSendByTeacherId(@Param("teacherId") Long teacherId, Pageable pageable);

    // 학생 받은쪽지 페이징 조회
    @Query("SELECT m FROM MessageEntity m WHERE m.studentEntity.id = :studentId and m.sender = 'T'")
    Page<MessageEntity> findAllReceiveByStudentId(@Param("studentId") Long studentId, Pageable pageable);

    // 학생 보낸쪽지 페이징 조회
    @Query("SELECT m FROM MessageEntity m WHERE m.studentEntity.id = :studentId and m.sender = 'S'")
